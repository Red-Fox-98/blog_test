...
//        private readonly LoggerInterface $logger
    ...
    public function importNews(?int $count = null, bool $dryRun = false): void
    {
        ...
        $crawler->filter('h4.my-0 > a')->each(function (Crawler $crawler) use (&$texts, $count) {
            if($count && count($texts) >= $count) return;
            $texts[] = [
                'title' => $crawler->text(),
        ...
        foreach ($texts as &$text) {
            $scrawler = new Crawler($this->httpClient->get($text['href']));
            $scrawlerBody = $scrawler->filter('div.grid-cols-article-mobile')->first();
            $text['text'] = $scrawlerBody->text();
